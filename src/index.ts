...
// Create server function (required for Smithery)
export default function createServer({
                ...
                if (!hfToken) {
                    return {
                        content: [
                            {
                                type: 'text',
                                text: '오류: HF_TOKEN이 설정되지 않았습니다. Smithery 설정에서 HF_TOKEN을 구성해주세요.'
                            }
                        ],
                        isError: true
                    }
    ...
    // Return the MCP server object (required for Smithery)
    return server.server
}

// Local development support (optional - for testing without Smithery)
